class state():
    ...
    атрибуты:
    -----------
    debug - для прохождения теста на шах и мат
    ch_b - список списков для построения доски и сохранения текущих позиций фигур
    white - очерёдность хода
    log - вхождения
    методы:
    -----------
...
class Moving():
    ...
    ch_b - доска
    st_r - cтартовый ряд 
    st_c - стартовая колонка
    fin_r - финальный ряд
    fin_c - финальная колонка
    m0ving_piece - передвижение фигуры
    capture - съедание фигуры
    p2q - при дохождении пешки до края поля превращение её в иную фигуры по выбору
    castling - рокировка
    '''
